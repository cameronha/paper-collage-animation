...
TEXT_SCENE_PROMPT = (
    ...
    "rendered as a {style} "
    "COMPOSITION: the artwork fills the whole frame edge to edge, full bleed, with no page "
    "margin and no inset panel. PULL THE CAMERA BACK — this is a medium or wide shot, "
    "NEVER a tight close-up. Show enough of the room or setting that the subject occupies "
    "well under half the frame height, with real visible space above, below and around "
    "them. Place the subject in the MIDDLE THIRD of the frame horizontally, so a square "
    "crop taken from the centre keeps them whole. The far left and far right, and the "
    "space around the subject, show ordinary background — furniture, walls, the room — "
    "never empty space, and nothing the viewer needs to see. "
